...
import { editContactPage_cms, editPagesPrivacyPolicy_cms, getBlog_cms, getPrivacy_cms, updateBlog_cms, updateUser } from "../../../User_Management/features/userSlice";
...
const CMSPrivacyPolicy = ({ setActiveTab, setExpand }) => {

  setActiveTab("pages");
  setExpand("contentManagement")
  const head = "Privacy Policy";
  ...
  const termsData = useSelector((state) => state.userManagement.getPrivacy_cms);
  useEffect(()=>{setPrivacy(termsData.content)},[termsData])
...
//   }, [blogData])
  ...
  const handleSubmit = async (event) => {
    ...
    formData.append("content", privacy);
    ...
    setLoading(true);
    await dispatch(editPagesPrivacyPolicy_cms(formData));
    setLoading(false);
    // navigate("/home/blogs")
          ...
          <div className="bg-[#EEEEEE] p-5 rounded-md drop-shadow-md borders mt-5 ">
            ...
            <div className="mr-6 my-5">
              ...
              <div className=" bg-white mt-2">
                <ReactQuill value={privacy} onChange={handlePrivacy} theme="snow" />
              </div>
            </div>
...
export default CMSPrivacyPolicy;
